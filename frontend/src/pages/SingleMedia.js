    ...
    return (
        ...
        <div style={{paddingBottom: '1000px'}}>
            ...
            >
                ...
                ></div>
                <h2 style={{ color: 'white', zIndex: '99', margin:'0.5em' }}>Let's Watch</h2>
                    <div style={{display:'flex', color: 'white', zIndex: '99'}}>
                        <div style={{position:'relative', display:'flex', alignItems:'center', margin:'auto'}}>
                            <img style={{width:'100px'}} src={`https://image.tmdb.org/t/p/original${currentMedia.poster_path}`} />
                            <FrontPageRatingBubble rating={Math.round(currentMedia.vote_average*10)} />
                        </div>                    
                        <div style={{margin:'0 10px', width:'65%'}}>
                        {currentMedia.title ? <h4  style={{margin:0}}>{currentMedia.title}</h4> : <h2 style={{margin:0}}>{currentMedia.name}</h2>}
                        {currentMedia.release_date ? <i style={{margin:0}}>{currentMedia.release_date.replaceAll('-','/')}</i> : <i style={{margin:0}}>{currentMedia.first_air_date.replaceAll('-','/')}</i>}
                        <p style={{margin:'0'}}>{currentMedia.genres.map( genre => <i>{genre.name} </i>)}</p>
                        <Button
                            onClick={()=>test()}
                            variant="contained"
                            style={{
                        ...
                        <div className='frame' style={{display:'flex', overflow:'scroll', padding:'10px', maxWidth:'350px', alignItems:'start',}}>
                            {cast ?  cast.map(c =>
                                <div style={{display:'flex', flexDirection:'column', margin:'0 10px'}}>
                                    ...
                                    <b style={{margin:0}}>{c.character}</b>
                                </div>
                            ):null}
                        </div>
                    </div>
